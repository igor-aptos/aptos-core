...
use aptos_aggregator::aggregator_extension::{extension_error, AggregatorID, SnapshotValue};
...
pub(crate) fn aggregator_value_field_as_id(value: u128) -> PartialVMResult<AggregatorID> {
    Ok(AggregatorID::ephemeral(u128_to_u64(value)?))
}

pub(crate) fn aggregator_snapshot_value_field_as_id(value: SnapshotValue) -> PartialVMResult<u64> {
    match value {
        SnapshotValue::Integer(v) => u128_to_u64(v),
